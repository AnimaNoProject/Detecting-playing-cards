...
obenzulinks = [firstcorner;secondcorner];
linkszuunten = [secondcorner;fourthcorner];
untenzurechts = [fourthcorner;thirdcorner];
rechtszuoben = [thirdcorner;firstcorner];

distOL = round(pdist(obenzulinks,'euclidean'));
distLU = round(pdist(linkszuunten,'euclidean'));
distUR = round(pdist(untenzurechts,'euclidean'));
distRO = round(pdist(rechtszuoben,'euclidean'));

distarray = [distOL,distLU,distUR,distRO];


%Wir suchen nach den 2 kürzesten wegen
shortestpath = distOL;
longestpath = distOL;
