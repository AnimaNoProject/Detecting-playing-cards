...
close all;
% Original Image
target = imread('Output/Spiel 2_7.jpg');
template = imread('input/Datensaetze/Templates/herz.png');
...
% targetHeight = size(target, 1); %height of the Original image
% targetWidth = size(target, 2); %width of the Original image
% template = imresize(template, [targetHeight targetWidth]);
...
corr_farbe = tmc(target, template);
